                                ...
                                size_t count, loff_t* position)
{
    ...
    //i.e. only allow reading up to buffer size
    count = MIN((*position+count), (_helloWorldSize - *position));

    if(copy_to_user(userBuffer, _helloWorldString + *position, count) != 0 )
      return -EFAULT;
